...
class EconomicCenter:
    ...
    def __init__(self, tax_policy: TaxPolicy = None, category_profit_margins: Dict[str, float] = None):
        ...
        # 💰 Product profit margin configuration (based on 12 major categories of Daily Category)
        # Generated by GPT-5, based on industry实际情况和市场竞争程度
        # 🔧 Fix: if None is passed, initialize default configuration (similar to SimulationConfig.__post_init__)
        ...
        self.firm_innovation_events: List[FirmInnovationEvent] = []  # [firm_id: innovation_events, month: month] 创新事件历史记录
        ...
        # 📉 未满足需求统计（用于“缺货→补货”闭环）
        # 结构：{month: {"product_id@firm_id": {"attempts": float, "qty_requested": float, "qty_short": float}}}
    ...
    def update_product_prices_based_on_sales(
        ...
        for owner_id, products in self.products.items():
            if owner_id in self.firm_id:  # 只处理真正的公司
                processed_owners += 1
                for product in products:
...
"""
            ...
            try:
                ...
                print(f"🤖 企业 {firm.firm_id} LLM决策: 研发比例={research_share:.1%} "
    ...
    async def _execute_labor_based_production(
        ...
        try:
                        ...
                        try:
                            ...
                            if self.enable_innovation_module:
                                ...
                                if innovation_config and innovation_config.get('enable_innovation_module', False):
                                    ...
                                    print(
                                        f"🔬 企业 {firm.firm_id} 创新: 研发有效劳动力={research_eff:.2f}, "
            ...
            for firm_id, employee_count in companies_with_employees.items():
                ...
                firm_labor_production[firm_id] = company_output  # 记录该企业的劳动力生产量
    ...
    async def update_prices_innovation_arrival(self, firm_id: str, gamma: float = 1.2, month: int = 0):
        ...
        print(f"🔬 公司 {firm_id} {month}月劳动力因素变化 {old_labor_production} -> {new_labor_production}")
    ...
    async def update_product_attributes_innovation_arrival(self, firm_id: str, gamma: float = 1.2, month: int = 0):
        ...
        if updated_products > 0:
            print(f"🔬 公司 {firm_id} {month}月商品属性提升: 放大系数={gamma}, 受影响商品={updated_products} 件")
            self.add_innovation_event(
                firm_id=firm_id,
                month=month,
    ...
    async def update_profit_margin_innovation_arrival(self, firm_id: str, gamma: float = 1.2, month: int = 0):
        ...
        print(f"🔬 公司 {firm_id} {month}月毛利率变化 {old_profit_margin} -> {new_profit_margin}")
    ...
    async def _calculate_company_labor_production(
        ...
        for product in company_products:
            ...
            # 计算优先级分数（基于销量和库存水平）
            # 使用 (product_id, firm_id) 作为key查找销量数据
            if sales_key in sales_data:
                # 有销售记录：基于销量计算优先级
                sales_info = sales_data[sales_key]
